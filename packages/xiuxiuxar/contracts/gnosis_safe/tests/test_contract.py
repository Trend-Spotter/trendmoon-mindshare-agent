...
from packages.xiuxiuxar.contracts.gnosis_safe.contract import (
    SAFE_CONTRACT,
    GnosisSafeContract,
)
...
class BaseContractTest(BaseGanacheContractTest):
    ...
    def owners(cls) -> list[str]:
        """Get the owners."""
        return [Web3.to_checksum_address(t[0]) for t in cls.key_pairs()[: cls.NB_OWNERS]]
...
class BaseContractTestHardHatSafeNet(BaseHardhatGnosisContractTest):
    ...
    def _verify_safe_tx(
        ...
        return self.contract.verify_tx(
            ...
            data=data,
            signatures_by_owner={
                self.deployer_crypto.address.lower(): signatures_by_owners[self.deployer_crypto.address]
            },
        )["verified"]
    ...
    def _get_raw_safe_tx(
        ...
        ledger_api = cast(EthereumApi, self.ledger_api)
        contract_address = cast(str, self.contract_address)
        return self.contract.get_raw_safe_transaction(
            ledger_api=ledger_api,
            contract_address=contract_address,
...
class TestDeployTransactionHardhat(BaseContractTestHardHatSafeNet):
    ...
    def test_deployed(self) -> None:
        ...
        assert all(key in result for key in expected_deploy_tx_fields), f"Error, found: {result}"
    ...
    def test_exceptions(
        ...
        with pytest.raises(ValueError, match="Client does not have any funds"):
            ...
            self.contract.get_deploy_transaction(
                ledger_api=self.ledger_api,
                deployer_address=crypto_registry.make(EthereumCrypto.identifier).address,
                owners=self.owners(),
                threshold=int(self.threshold()),
            )
    ...
    def test_revert_reason(
        ...
        with mock.patch.object(self.ledger_api.api.eth, "call", new=_raise_solidity_error):
            reason = self.contract.revert_reason(self.ledger_api, "contract_address", cast(TxData, tx))
        ...
        with (
            mock.patch.object(self.ledger_api.api.eth, "call"),
            pytest.raises(ValueError, match=f"The given transaction has not been reverted!\ntx: {tx}"),
        ):
            self.contract.revert_reason(self.ledger_api, "contract_address", cast(TxData, tx))
    ...
    def test_get_incoming_transfers(self) -> None:
        ...
        assert data[0]["sender"] == self.deployer_crypto.address, f"{data[0]['sender']} should be the sender"
...
class TestRawSafeTransaction(BaseContractTestHardHatSafeNet):
    ...
    def test_run(self) -> None:
        ...
        value = 0
        data = b""
        sender = crypto_registry.make(EthereumCrypto.identifier, private_key_path=ETHEREUM_KEY_PATH_1)
        assert sender.address == self.owners()[1]
        receiver = crypto_registry.make(EthereumCrypto.identifier, private_key_path=ETHEREUM_KEY_PATH_2)
        assert receiver.address == self.owners()[2]
        fourth = crypto_registry.make(EthereumCrypto.identifier, private_key_path=ETHEREUM_KEY_PATH_3)
        ...
        signatures_by_owners = {
            crypto.address: crypto.sign_message(b_tx_hash, is_deprecated_mode=True)[2:] for crypto in cryptos
        }
        ...
        assert all(key in tx for key in EXPECTED_TX_KEYS), "Missing key"
        ...
        verified = self._verify_safe_tx(tx_hash, value, data, receiver.address, signatures_by_owners)
    ...
    def test_verify_negative(self) -> None:
        ...
        verified = self.contract.verify_tx(
            ...
            owners=("",),
            to_address=crypto_registry.make(EthereumCrypto.identifier, private_key_path=ETHEREUM_KEY_PATH_1).address,
            value=0,
            data=b"",
            signatures_by_owner={},
        )["verified"]
    ...
    @mock.patch.object(
        ...
        return_value=AttributeDict(
            {
                "accessList": [],
                "blockHash": HexBytes("0x8543592f08d1d9e6d722ba9d600270d7e7789ecc9b66f27ca81b104df9c5dd4a"),
                "blockNumber": 31190129,
                "chainId": "0x89",
                "from": "0x5eF6567079c6c26d8ebf61AC0716163367E9B3cf",
                "gas": 270000,
                "gasPrice": 36215860217,
                "hash": HexBytes("0x09d5be525caea564b2d4fd31af424c8f0414a9b270937a1bee29167a883e6ce5"),
                ...
                "nonce": 2231,
                "r": HexBytes("0x5d5d369d5fc30c5604d974761d41b08118120eb94fd65a827bab1f6ea558d67c"),
                "s": HexBytes("0x12f68826bd41989335e62d43fd36547fe171ad536b99bc93766622438d3f8355"),
...
class TestOwnerManagement(BaseContractTestHardHatSafeNet):
    ...
    def test_remove(self) -> None:  # pylint: disable=too-many-locals  # noqa: PLR0914
        """Test owner removal."""
        assert self.contract_address is not None
        ...
        verified = self._verify_safe_tx(tx_hash, value, data, self.contract_address, signatures_by_owners)
        assert verified, f"Not verified: {verified}"
        time.sleep(1)
        ...
        assert remove_events[0].get("owner") == owner_to_be_removed, "a different owner than expected was removed"

    def test_swap(self) -> None:  # pylint: disable=too-many-locals  # noqa: PLR0914
        """Test owner swapping."""
        assert self.contract_address is not None
        ...
        assert remove_events is not None, "a RemovedOwner event was expected"
        assert len(remove_events) == 1, "1 RemovedOwner event was expected"
        assert remove_events[0].get("owner") == old_owner, "a different owner than expected was removed"
