...
from Crypto.Hash import keccak  # nosec  # noqa: S413
from eth_abi.abi import default_codec  # nosec


def encode(typ: t.Any, arg: t.Any) -> bytes:
    """Encode by type."""
    encoder = default_codec._registry.get_encoder(  # pylint: disable=protected-access  # noqa: SLF001
        typ
    )
    return encoder(arg)


def to_string(value: bytes | str | int) -> bytes:
...
def sha3(seed: bytes | str | int) -> bytes:
    """Calculate SHA3-256 hash."""
...
def create_struct_definition(name: str, schema: list[dict[str, str]]) -> str:
    """Create method struction defintion."""
    schema_types = [(schema_type["type"] + " " + schema_type["name"]) for schema_type in schema]
    return name + "(" + ",".join(schema_types) + ")"


def find_dependencies(name: str, types: dict[str, t.Any], dependencies: set) -> None:
    """Find dependencies."""
...
def create_schema(name: str, types: dict) -> str:
    ...
    return create_struct_definition(clean_name, types[clean_name]) + "".join(dependency_definitions)
...
def encode_value(data_type: str, value: t.Any, types: dict) -> bytes:
    """Encode value."""
    if data_type == "string":
    ...
    if data_type.endswith("]"):
        arrayType = data_type[: data_type.index("[")]  # noqa: N806
        return encode(
            "bytes32",
            sha3(b"".join([encode_data(arrayType, arrayValue, types) for arrayValue in value])),
        )
    return encode(data_type, value)


def encode_data(name: str, data: dict[str, dict[str, str]], types: dict) -> bytes:
    """Encode data."""
    return create_schema_hash(name, types) + b"".join(
        [encode_value(schema_type["type"], data[schema_type["name"]], types) for schema_type in types[name]]
    )


def create_struct_hash(name: str, data: dict[str, dict[str, str]], types: dict) -> bytes:
    """Create struct hash."""
    return sha3(encode_data(name, data, types))


def encode_typed_data(data: dict[str, t.Any]) -> bytes:
    """Encode typed data."""
    types = t.cast(dict, data.get("types"))
    primary_type = t.cast(str, data.get("primaryType"))
    domain = t.cast("dict[str, dict[str, str]]", data.get("domain"))
    message = t.cast(dict[str, t.Any], data.get("message"))
